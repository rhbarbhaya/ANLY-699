...
bibliography      : ["/Users/rush/Documents/ANLY-699/Final Paper/ref.bib"]
...
library("papaja")
r_refs("/Users/rush/Documents/ANLY-699/Final Paper/ref.bib")
```
...
@abernathy1975technology observes the evolution of manual processes. A process that starts as simple logic; evolves into a complex one over time. This complex process generates inefficiencies, and machines are employed to bring back the lost inefficiencies. The author did not account for how these trends are observed in different industrial sectors. @evangelista2012impact balances out the corporate perspective and speaks for human evolution. As robots take on menial jobs, humans find a more specialized roles. Those specialized roles spikes growth and knowledge. Similarly, @bainbridge1982ironies describes how automation can work in tandem with humans. Humans can take more managerial roles and let machines handle the rule-based task.
...
## Transportation
The transportation industry is moving towards automated driving systems. (@rice2019driverless) Waymo and Tesla are leading that, among others. They are already saving lives, and @lala2020autonomous shows that the better the automated systems get, the fewer losses to human lives. @schwall2020waymo, their report mentions the automated systems have already made ways in saving lives. These papers do not talk about what happens if there is an automated car causes an accident. Until driverless cars or self-driving vehicles become a mainstay, @ward2000automation proposes developing an Adaptive Cruise Control system that helps reduce errors and accidents. A need for this cruise control arises because humans have an inherent tendency to make errors as they work on multiple tasks at a time. Having a dedicated machine would help in preventing the loss of lives. The paper does not talk about a merger of these technologies.

## Manufacturing
The manufacturing industry has utilized robots and artificial intelligence most all the industries. @jamsa2007future talks about how modern industries utilize automation to deliver a reliable product. They use machines anywhere from research and development to marketing the product. The chemical industry is the biggest one. However, the authors missed extending those mechanical knowledge/skills to other industrial sectors.
...
## Tools of automation
@R-base with @R-papaja are used to create this paper and @python.org_2021 for modeling and plotting graphs for these analyses. \
...
### Flights in the USA
For understanding the automation industry, it is important to understand the number of flights that have taken off from the land of the USA. It, directly and indirectly, gives us a sense of how the population perceives the aviation industry as a whole. One of the factors for understanding safety in the aviation industry would be utilizing the services. Economics and logistics are also important factors in the industry, but they do not fall within the scope of this research. The data for getting the number of flights in the USA was downloaded from @btstransstats, and the key for the data was derived from @AirlineI37. The description of the table \@ref(tab:aviation-flights) \

```{r aviation-flights, fig.align='center'}
flights_table <- data.frame(
  'Column' = c('Carrier', 'Carrier Name', 'Origin Airport ID', 'Origin', 'Destination Airport ID', 'Destination', 'Year', 'Month'),
  'Context' = c('Unique value of the airline carrier', 'Full name of the airline carrier', 'Code of the airport from where the aircraft took flight', 'Name of the place the airline took off', 'Code of the airport where the aircraft landed', 'Full name of the destination airport', 'Timestamp of when the aircraft took flight', 'Timestamp of when the aircraft took flight'),
  'Datatype' = c("string", 'string', 'interger', 'string', 'interger', 'string', 'date(YYYY)', 'date(MM)')
)
apa_table(
  flights_table,
...
knitr::include_graphics("./graphs/Count of flights.png", dpi = 108)
```

```{r aviation-jobs-schema}
aviation_jobs_table <- data.frame(
  'column' = c('ID', 'Year', 'Period', 'Label', 'Value', ''),
  'Context' = c('Unique identifier for each year', 'Year for the statictic', 'Month of the statictic', 'Month and Year combined for label', 'Year "1" acts as benchmark and subsequest year shows the', 'percent increase/decrease in employement numbers'),
  'format' = c('string', 'date(YYYY)', 'string(date(MM))', 'string', 'float', '')
)
apa_table(
  aviation_jobs_table,
...
knitr::include_graphics("./graphs/Jobs in the aviation industry.png", dpi = 108)
```

```{r aviation-incident-plot, fig.cap="Trend of injuries across time", warning=FALSE, echo=FALSE, fig.align='center'}
knitr::include_graphics("./graphs/flight_injuries.png", dpi = 108)
```
...
To measure how automation has affected the aviation industry, it is to look at the jobs in the aviation industry. The Bureau of Labor Statistics @BLSDataV82 provides the data for this analysis. The data schema is shown in the table \@ref(tab:aviation-jobs-schema). There are no `NULL` or `empty` values in the data; hence cleanup is not required. This data comes directly from the Bureau of Labor Statistics and is not scoped for outliers. \
...
After any incident in any aviation industry, a new safety standard is observed. New automation opportunity arises from these safety standards. Auto-pilot was introduced for long flights to relieve pilots from fatigue. Take-off and landing assistance were introduced to combat climatic factors at the airports. Overall the safety standards increase following incidents. @Accident29 provides us with the incident figures with the count of fatalities and injuries. The overall dataset facts are stated in the table \@ref(tab:aviation-incidents-schema) \

The dataset contains `NULL` in the integer columns. Those are equated to `0` for calculated columns. `Total Ground Injures` and `Total Flight Injuries` are calculated columns to understand the trend of injuries across `year,` displayed in figure \@ref(fig:aviation-incident-plot) \
